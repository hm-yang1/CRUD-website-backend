...
func GetPostsHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Pagination
	addedPaginationQuery, args := addPagination(addedSortingQuery, pageStr, perPageStr)
...
func CreatePostHandler(w http.ResponseWriter, r *http.Request) {
	...
	result, err := models.DataBase.Exec("INSERT INTO posts (username, tag, title, description, upvotes, datetime) VALUES (?, ?, ?, ?, ?, ?)", createdPost.Username, string(tagsJSON), createdPost.Title, createdPost.Description, createdPost.Upvote, createdPost.Datetime)
	if err != nil {
...
func EditPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err = models.DataBase.Exec("UPDATE posts SET tag = ?, title = ?, description = ? WHERE postid = ?", tagsJSON, updatedPost.Title, updatedPost.Description, postID)
	if err != nil {
...
func DeletePostHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err = models.DataBase.Exec("DELETE FROM posts WHERE postid = ?", postID)
	if err != nil {
...
func getPostById(postID int) (models.Post, error) {
	fmt.Println(postID)
	row := models.DataBase.QueryRow("SELECT * FROM posts WHERE postid = ?", postID)
...
func addPagination(query string, pageStr string, perPageStr string) (string, []interface{}) {
	var args []interface{}
	if pageStr != "" && perPageStr != "" {
		...
		query += " LIMIT ?, ?"
		args = append(args, (pageNumber-1)*itemsPerPage, itemsPerPage)
