...
func insertTagsForPost(postID int64, tagNames []string) error {
	...
	for _, tagName := range tagNames {
		var tagID int
		err := models.DataBase.QueryRow("SELECT tagsid FROM tags WHERE name = ?", tagName).Scan(&tagID)
		if err != nil {
			fmt.Println(err)
			return err
		}
	...
	for _, tagID := range tagIDs {
		_, err := models.DataBase.Exec("INSERT INTO posttags (posttags_tagid, posttags_postid) VALUES (?, ?)", tagID, postID)
		if err != nil {
			return err
		}
	}
...
func deleteTagsForPost(postID int) error {
	...
	_, err = tx.Exec("DELETE FROM posttags WHERE posttags_postid = ?", postID)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func getFilteredPosts(tagNames []string, sortBy string, pageStr string, perPageStr string) ([]models.Post, error) {
	...
	if pageStr != "" && perPageStr != "" {
		...
		query += " LIMIT ?, ?"
		postIDArgs = append(postIDArgs, (pageNumber-1)*itemsPerPage, itemsPerPage)
	}
